...
<Meta title="Addon/Docs/Controls" decorators={[moduleMetadata({ declarations: [Button] })]} />
...
<Story name="with controls" controls={{ text: { type: 'text',label: 'Button text', value: 'Hello Button'}}}>
  {({ text }) => {
    return {
    template: `<storybook-button-component [text]="text" (onClick)="onClick($event)"></storybook-button-component>`,
    props: {
